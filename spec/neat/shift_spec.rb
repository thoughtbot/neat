...
describe "@include shift()" do
  before(:all) do
    ...
    ParserSupport.parse_file("shift-fixed")
  end
  ...
  context "in a fixed grid" do
    context "with no argument" do
      it "shifts element one column in the default direction" do
        expect('.shift-fixed-default').to have_rule('margin-left: 82px')
      end
    end

    context "with argument (2)" do
      it "shifts element 2 columns in the default direction" do
        expect('.shift-fixed-positive').to have_rule('margin-left: 164px')
      end
    end

    context "with argument (-3)" do
      it "shifts element 3 colums in the opposite direction" do
        expect('.shift-fixed-negative').to have_rule('margin-left: -246px')
      end
    end

    context "when layout direction changes" do
      it "shifts element in the corresponding direction" do
        expect('section .shift-fixed-negative').to have_rule('margin-right: -246px')
      end
    end
  end
end
